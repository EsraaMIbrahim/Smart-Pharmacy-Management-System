...
_Advanced Analytics (Led by Abanoub M.)_
...
[1] A client can book a consultancy session with a pharmacist.

[2] Pharmacy admin can define the state of each session, and assign sessions to pharmacists

[3] a pharmacist can change the state of each session (pending, completed, confirmed, and cancelled) he also can see patient's data, and the reason of the session.
