...
function App() {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post(
        "https://localhost:7168/api/Auth/login",
        {
  ...
  const handleRegister = async () => {
    try {
      ...
      alert(
        `✅ Registration Successful for ${response.data.username || loginCredentials.username}! You can now login.`,
      );
      ...
      const message =
        ...
        "Username or Phone might already exist.";
  ...
  const toggleActiveStatus = async (medicine) => {
    ...
    const currentStatus =
      medicine.IsActive !== undefined ? medicine.IsActive : medicine.isActive;
  ...
  const findAlternatives = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/FindAlternatives/${encodeURIComponent(searchAlt)}`,
      );
      // DOUBLE CHECK: Filter out any inactive ones in React too
      const activeAlts = response.data.filter(
        (alt) => (alt.IsActive ?? alt.isActive) !== false,
      );
  ...
  const checkSafety = async () => {
    try {
      ...
      const response = await axios.get(
        `https://localhost:7168/api/Medicines/CheckInteractionByMedicine?med1=${med1}&med2=${med2}`,
      );
  ...
  const addPatient = async () => {
    ...
    try {
      ...
      alert("Error: Is this phone number already registered?");
    }
  };
  ...
  const deletePatient = async (id) => {
    if (
      window.confirm(
        "Are you sure you want to delete this patient profile? This will not delete their purchase history but will unbind it.",
      )
    ) {
      try {
        ...
        alert(
          "❌ Error deleting patient. They might have active records linked to them.",
        );
      }
    }
  };
  ...
  const deleteSupplier = async (id) => {
    ...
    if (
      window.confirm(
        "⚠️ Are you sure? This will delete the supplier and all their linked purchase history!",
      )
    ) {
      try {
        ...
        console.error("Delete Error:", error);
        alert(
          "❌ Cannot delete: Please ensure the C# Delete method handles cascading history.",
        );
      }
    }
  };
  ...
  const fetchPurchaseHistory = async () => {
    try {
      const response = await axios.get(
        "https://localhost:7168/api/Suppliers/PurchaseHistory",
      );
  ...
  const fetchSalesHistory = async () => {
    try {
      const response = await axios.get(
        "https://localhost:7168/api/Patients/AllSales",
      );
  ...
  const updateSupplier = async () => {
    try {
      await axios.put(
        `https://localhost:7168/api/Suppliers/${editSupplierId}`,
        {
      ...
      if (error.response?.status === 405) {
        alert(
          "❌ Error 405: The C# Controller is missing the [HttpPut] method.",
        );
      } else {
  ...
  const disableExpiredMedicines = async () => {
    const expiredOnes = medicines.filter(
      (m) =>
    ...
    if (expiredOnes.length === 0)
      return alert("✅ No active expired medicines found!");

    if (
      window.confirm(
        `Found ${expiredOnes.length} expired items. Disable all for safety?`,
      )
    ) {
      try {
        ...
        cart.map((item) =>
          item.id === medId
            ? {
                ...
                totalPrice: (item.quantity + 1) * medPrice,
              }
            : item,
        ),
      );
  ...
  const handleCheckout = async () => {
    ...
    if (userRole === "Client" && !currentUserId) {
      alert(
        "❌ Session Error: Please log out and log back in to refresh your User ID.",
      );
      return;
    }
    ...
    if (userRole === "Client") {
      ...
      const phone = prompt(
        "Enter Patient Phone Number (Leave blank for Anonymous):",
      );
    ...
    try {
      for (const item of cart) {
        if (userRole === "Client") {
          ...
          await axios.post(
            "https://localhost:7168/api/OnlineOrders",
            orderPayload,
          );
        } else {
          await axios.post(
            "https://localhost:7168/api/Patients/RecordPurchase",
            {
      ...
      alert(
        userRole === "Client"
          ? `✅ Order confirmed! Delivering to: ${finalAddress}`
          : "✅ Sale Complete!",
      );
    } catch (error) {
  ...
  const fetchMyHistory = async () => {
    ...
    if (!currentUserId) {
      console.warn(
        "Cannot fetch history: User ID is missing. Try logging in again.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await axios.get(
        `https://localhost:7168/api/OnlineOrders/MyHistory/${currentUserId}`,
      );
  ...
  const salesCountMap = React.useMemo(() => {
    return salesHistory.reduce((map, record) => {
      ...
      const name = (record?.medicineName || record?.MedicineName || "")
        .toLowerCase()
        .trim();
  ...
  const getSmartDiscount = (expiryDate) => {
    ...
    if (daysUntilExpiry <= 5) return 0.4; // 40% (Last chance to break even)
    if (daysUntilExpiry <= 14) return 0.2; // 20% (Standard promotion)
    if (daysUntilExpiry <= 30) return 0.1; // 10% (Early incentive)
  ...
  const handleApplyInventoryDiscount = async (med) => {
    ...
    if (
      window.confirm(
        `Apply ${percentage}% smart discount? New price: ${suggestedPrice} EGP`,
      )
    ) {
      try {
              ...
              {userRole === "Client" ? (
                ...
                <>
                  <button
                    ...
                    style={{
                      backgroundColor:
                        view === "client_store" ? "#28a745" : "transparent",
                      border: "none",
                      color: "white",
                      padding: "8px 15px",
                      borderRadius: "5px",
                  ...
                  <button
                    ...
                    style={{
                      marginLeft: "10px",
                      backgroundColor:
                        view === "my_orders" ? "#28a745" : "transparent",
                      border: "none",
                      color: "white",
                      padding: "8px 15px",
                      borderRadius: "5px",
                ...
                <>
                  <button
                    ...
                    style={{
                      marginRight: "10px",
                      backgroundColor:
                        view === "inventory" ? "#007bff" : "transparent",
                      border: "none",
                      color: "white",
                      cursor: "pointer",
                      fontWeight: "bold",
                  ...
                  <button
                    ...
                    style={{
                      marginRight: "10px",
                      backgroundColor:
                        view === "patients" ? "#007bff" : "transparent",
                      border: "none",
                      color: "white",
                      cursor: "pointer",
                      fontWeight: "bold",
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor:
                        view === "suppliers" ? "#007bff" : "transparent",
                      border: "none",
                      color: "white",
                      cursor: "pointer",
                      fontWeight: "bold",
          ...
          {view === "inventory" && userRole !== "Client" && (
            <>
                  ...
                  >
                    ...
                    >
                      {medicines
                        .filter(
                          (m) =>
                            getSmartDiscount(m.ExpiryDate || m.expiryDate) >
                              0 &&
                            (m.Price || m.price) >=
              ...
              {(userRole === "Admin" || userRole === "Pharmacist") && (
                ...
                >
                  <h3>
                    {isEditing ? "📝 Modify Medicine" : "➕ Add New Inventory"}
                  </h3>
                  <div
                    style={{
                      display: "grid",
              ...
              >
                ...
                <div
                  style={{
                    marginTop: "10px",
                    fontWeight: "bold",
                    color: safetyMessage.includes("DANGER") ? "red" : "green",
                  }}
                >
                  {safetyMessage}
                </div>
              ...
              >
                <input
                  ...
                  style={{ flex: 3, padding: "12px", borderRadius: "5px" }}
                />
                <button
                  onClick={handlePrint}
              ...
              >
                <h3
                  style={{ color: "#28a745", marginTop: 0, fontSize: "20px" }}
                >
                ...
                >
                  ...
                  <div style={{ textAlign: "right" }}>
                    <h4 style={{ margin: "10px 0" }}>
                      Grand Total:{" "}
                      {cart.reduce((sum, i) => sum + i.totalPrice, 0)} EGP
                    </h4>
                    <button
                ...
                <tbody>
                  {filteredMedicines.map((med) => {
                    ...
                    const basePrice = Number(
                      med?.BasePrice ?? med?.basePrice ?? currentPrice,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          ...
                          {salesCountMap[medName.toLowerCase().trim()] >=
                            TRENDING_THRESHOLD && (
                            <span
                        ...
                        <td style={{ minWidth: "180px", textAlign: "center" }}>
                          ...
                          >
                            ...
                            currentPrice >
                              (
                                basePrice *
                                (1 - getSmartDiscount(med.expiryDate))
                              ).toFixed(2) ? (
                                  ...
                                  <div style={{ display: "flex", gap: "5px" }}>
                                    ...
                                    userRole === "Pharmacist" ? (
                                      <>
                                        <button
                                          onClick={() =>
                                            handleApplyInventoryDiscount(med)
                                          }
                                          style={{
                                            flex: 1,
                                            backgroundColor: "#28a745",
            ...
            >
                    ...
                    >
                      ...
                      <div
                        style={{
                          marginTop: "10px",
                          fontWeight: "bold",
                          color: safetyMessage.includes("DANGER")
                            ? "red"
                            : "green",
                        }}
                      >
                  ...
                  >
                    {filteredMedicines
                      ...
                      .map((med) => {
                        ...
                        const medName =
                          med?.Name || med?.name || "Unknown Medicine";
                        const medCat =
                        ...
                        return (
                          ...
                          >
                            ...
                            <div style={{ marginBottom: "25px" }}>
                              ...
                              >
                                {currentPrice.toFixed(2)}{" "}
                                <small style={{ fontSize: "14px" }}>EGP</small>
                              </span>
                            </div>
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (
                                  window.confirm(
                                    `✅ ${medName} added! Go to checkout now?`,
                                  )
                                ) {
              ...
              {view === "my_orders" && (
                ...
                >
                  ...
                  <p style={{ color: "#666" }}>
                    Track your recent pharmacy purchases and delivery status.
                  </p>
                      ...
                      <tbody>
                        {myOrders.length > 0 ? (
                          ...
                          <tr>
                            ...
                            >
                              No orders found yet. Start shopping to see your
                              history!
                            </td>
                          </tr>
                        )}
          ...
          {view === "patients" && (
            ...
            >
              ...
              <div
                style={{
                  marginBottom: "20px",
                  backgroundColor: isEditingPatient ? "#fff3cd" : "#f8f9fa",
                  padding: "15px",
                  borderRadius: "8px",
              ...
              >
                ...
                <button
                  ...
                  style={{
                    backgroundColor: isEditingPatient ? "orange" : "#28a745",
                    color: "white",
                    padding: "8px 15px",
                    border: "none",
                    borderRadius: "5px",
                    ...
                    <tbody>
                      {selectedHistory.length > 0 ? (
                        ...
                        <tr>
                          ...
                          >
                            No records found in SQL for this patient.
                          </td>
                        </tr>
                      )}
                    </tbody>
            ...
            >
              ...
              >
                <h4
                  style={{
                    marginTop: 0,
                    color: isEditingSupplier ? "#007bff" : "#28a745",
                    marginBottom: "20px",
                  }}
                >
                  {isEditingSupplier
                ...
                >
                  <button
                    onClick={
                      isEditingSupplier ? updateSupplier : handleAddSupplier
                    }
                    style={{
                      flex: 1,
                      backgroundColor: isEditingSupplier
                        ? "#007bff"
                        : "#28a745",
                      color: "white",
                      padding: "12px",
              ...
              >
                ...
                >
                  <select
                    ...
                    onChange={(e) =>
                      setOrder({ ...order, medicineId: e.target.value })
                    }
                    style={{
                      padding: "10px",
                      borderRadius: "5px",
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setOrder({ ...order, costPrice: e.target.value })
                    }
                    style={{
                      padding: "10px",
                      borderRadius: "5px",
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <h3 style={{ borderBottom: "2px solid #f2f2f2" }}>
                    🚚 Active Suppliers
                  </h3>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      ...
                      >
                        ...
                        <th style={{ padding: "12px" }}>Office Address</th>{" "}
                        {/* NEW COLUMN */}
                          ...
                          <td style={{ padding: "15px", textAlign: "center" }}>
                            ...
                            >
                              {(userRole === "Admin" ||
                                userRole === "Pharmacist") && (
                                <button
                                  onClick={() => handleSupplierEditClick(s)}
                                  style={{
                      ...
                      {purchaseHistory.length > 0 ? (
                        purchaseHistory.map((h) => (
                          ...
                          >
                            ...
                            {/* Use orderDate to match the Shipment model */}
                            <td>
      ...
      {currentInvoice && (
        ...
        >
          ...
          >
            ...
            >
              Smart Pharmacy Receipt
            </h2>
            <p>
              <strong>Date:</strong> {currentInvoice.date}
            </p>
            <p>
              <strong>Patient:</strong> {currentInvoice.patient}
            </p>
            <hr />
            ...
            <h3 style={{ backgroundColor: "#f8f9fa", padding: "10px" }}>
              Total: {currentInvoice.total} EGP
            </h3>
            <p style={{ fontSize: "12px" }}>
              Thank you for choosing Smart Pharmacy!
            </p>
